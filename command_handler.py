...
class CommandHandler:
    ...
    def delete(self, chatId, command):
        ...
        if len(parts) == 1:  # list all the watches and alerts with an ID
            ...
            deleteList += ' \nTo delete an alert or watch, type /delete <ID> or /delete all'
            ...
            deleteID = parts[1]
            if len(deleteID)!= 4 and deleteID.lower() != 'all':
                self.api.sendMessage("Invalid ID, must be 4 characters long or 'all' value", chatId)
                return
            
            # if it is not valid hex then error and return
            try:
                if deleteID.lower() == 'all':
                    deleteID = 'all'
                else:
                    int(deleteID, 16)
            except:
...
# here we scan through all of them and find the one to delete
                    ...
                    for op in alerts[fsym]:
                        for tsym in alerts[fsym][op]:
                            for target in alerts[fsym][op][tsym]:
                                alertString = f'{fsym} {op} {target} {tsym}\n' 
                                hashOfAlert = get_id(chatId, target)[:4]
                                if deleteID == hashOfAlert or deleteID == 'all':
                                    if deleteID != 'all':
            ...
            if 'watches' in self.db:
                for watch in self.db['watches']:
                    if watch['chatId'] == chatId:
                        ...
                        if str(deleteID) == str(hashOfWatch) or deleteID == 'all':
                            if deleteID != 'all':
            ...
            if deleteID == 'all':
                self.db['watches'] = []
                self.db['alerts'][chatId] = {}
                self.api.sendMessage('All watches and alerts deleted', chatId)
                self.log.info('All watches and alerts deleted')
                return
    ...
    def dropby(self, chatId, command):
        parts = command.split()
        if len(parts) != 5:
            self.api.sendMessage("Invalid command: /dropby BTC 50% <n> [days|weeks|months|years]", chatId)
            return
